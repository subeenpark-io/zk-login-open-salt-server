...
function normalizeSaltServerBase(url: string): string {
  return url
    .replace(/\/+$/, "")
    .replace(/\/v1\/salt$/i, "")
    .replace(/\/v1$/i, "");
...
function normalizeProverBase(url: string): string {
  return url.replace(/\/+$/, "").replace(/\/v1$/i, "");
}

// Configuration
const SALT_SERVER_URL = normalizeSaltServerBase(
  process.env.SALT_SERVER_URL ||
    "http://zklogin-prod-alb-346534019.ap-northeast-2.elb.amazonaws.com"
);
...
const SPONSORED_TX_ENABLED =
  process.env.SPONSORED_TX_ENABLED !== "false" && SPONSOR_PRIVATE_KEY.length > 0;
